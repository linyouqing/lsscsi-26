...
"Usage: lsscsi   [--classic] [--device] [--generic] [--help] [--hosts]\n"
            ...
            "\t\t[--wwn] [<h:c:t:l>] [--rescan] [--forcerescan]\n"
"  where:\n"
...
"    --force_rescan|-f     rescan the device force\n"
"    --rescan|-r      rescan the device\n\n"
               ...
               const struct lsscsi_opt_coll * op)
{
        ...
        if (op->size) {
                ...
                strncpy(blkdir, buff, NAME_LEN_MAX);
                strncpy(tmppath,buff,NAME_LEN_MAX);
                value[0] = 0;
                if (type == 0 &&
                    block_scan(blkdir) &&
...
static void  osn_output(void)
{
	char value[NAME_LEN_MAX];
	memset(value, '\0', NAME_LEN_MAX);
	if (0 == output.devname) {
		strcpy(value, "Devname");
		printf("%-13s", value);
		memset(value, '\0', NAME_LEN_MAX);
	}
	if (0 == output.type) {
...
void
osn_rescan_device(struct lsscsi_opt_coll *opts)
{
    char cmd[OSN_COMMAND_LENGTH];

    memset(cmd, 0, OSN_COMMAND_LENGTH);
    snprintf(cmd, OSN_COMMAND_LENGTH, "scsi-rescan");

    if (filter.h != -1) {
        snprintf(cmd+strlen(cmd), OSN_COMMAND_LENGTH-strlen(cmd), " --hosts=%d", filter.h);
        if (filter.c != -1) {
            snprintf(cmd+strlen(cmd), OSN_COMMAND_LENGTH-strlen(cmd), " --channels=%d", filter.c);
            if (filter.t != -1) {
                snprintf(cmd+strlen(cmd), OSN_COMMAND_LENGTH-strlen(cmd), " --ids=%d", filter.t);
                if (filter.l != -1) {
                    snprintf(cmd+strlen(cmd), OSN_COMMAND_LENGTH-strlen(cmd), " --luns=%d", filter.l);
                }
            }
        }
    }
    ...
    if (opts->force_rescan)
        snprintf(cmd+strlen(cmd), OSN_COMMAND_LENGTH-strlen(cmd), " --forcerescan");

    if (!opts->verbose)
        snprintf(cmd+strlen(cmd), OSN_COMMAND_LENGTH-strlen(cmd), " > /dev/null");
...
main(int argc, char **argv)
{
        ...
        while (1) {
                ...
                c = getopt_long(argc, argv, "cdfghHklLpPrstvVy:w", long_options,
                                &option_index);
                if (c == -1)
                        break;
...
//        printf("%d %d %d\n",output.device,output.devname,output.rev);
        if (do_rescan)
               	osn_rescan_device(&opts);
        else if (do_hosts)
                list_hosts(&opts);
        else if (do_sdevices)
                list_sdevices(&opts);
